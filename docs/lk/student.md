...
* При переходе по персональной ссылке регистрация произойдет автоматически и вы сразу попадете в личный кабинет. Последующий вход в личный кабинет нужен будет осуществлять только по этой ссылке.
* При переходе по общей ссылке возможны разные сценари работы механизма регистрации. Рассмотрим наиболее популярный:
    + На нулевом этапе потребуется указать свою группу или класс.
    ![](../_media/lk/reg-group.png)
    + На первом этапе потребуется выбрать свой портрет.
    ![](../_media/lk/reg-avatar.png)
    + На втором этапе нужно задать Фамилию и Имя.
    ![](../_media/lk/reg-data.png)
    + На последнем этапе будет предоставлена персональная ссылка доступа, которую рекомендуется либо продублировать на эл. почту, либо где-либо сохранить (например, в сохраненных сообщениях мессенджера).
