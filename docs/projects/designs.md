...
* Следующим этапом после отбора фотографий является "__Подготовка дизайнов__", который в свою очередь состоит из нескольких подэтапов. При этом сам процесс генерации альбомов запускается вручную.
* В первую очередь потребуется сформировать мастер-альбом, который будет содержать:
