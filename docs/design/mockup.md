...
description: В данном разделе содержится инструкция по подготовке макетов в формате PSD
sidebar_position: 1
---
# Подготовка макета
...
### Общая информация
:::info[Информация]
Воспользуйтесь нашими примерами макетов для [альбома и календаря-плаката](https://moyvipusknoy.ru/content/graduation_photobook_vertical_templates/vertical_templates.zip).
:::
...
* В макете недопустимо использовать эффекты прозрачности и деформации слоев без растрирования. А следующие эффекты работают с некоторыми ограничениями:
    + Обводка текста - без закругления, без прозрачности, тип обводки "Цвет", положение "Снаружи";
    + Тень текста - без прозрачности;
    + Обводка рамки для фото - без закругления, без прозрачности, тип обводки "Цвет", положение "Снаружи", только для фото прямоугольной формы.
...
Блочный текст может содержать любое количество строк и иметь выравнивание только по левому краю. В блочном тексте перенос строки должен осуществляться через Enter.
* Для создания макета можно использовать любые лицензионные шрифты. Однако, если шрифт не принадлежит коллекции __Google Fonts__, предварительно его нужно загрузить в систему и связать через механизм маппинга с названием шрифта в макете.
* Цвет текста может быть не только однотонным, но и градиентным. Для задания градиента необходимо использовать ключ ```[gradient: 45deg, rgba(0, 255, 0, 1) 10, rgba(255, 0, 0, 1) 90]```, где ```gradient: 45deg``` - это угол наклона градиента, ```rgba(0, 255, 0, 1)``` - это цвет, ```10``` - точка останова цвета. Цветов в градиенте должно быть не менее двух.
* Парение текста задается в формате ```[hover: 50 rgba(22, 22, 22, 1)]```, где ```hover: 50``` - значение интенсивности парения, ```rgba(22, 22, 22, 1)``` - это цвет.
* Фон текста задается в формате ```[text-background: padding: 50 radius: 50 rgba(222, 222, 0, 1)]```, где ```padding: 50``` - это внутренний отступ со всех сторон от текста, ```radius: 50``` - это степень скругления углов фона текста, ```rgba(222, 222, 0, 1)``` - это цвет фона.
...
![](../_media/design/photoWithoutDecor.png)
...
![](../_media/design/photoWithDecor.png)
#### Фотоколлажи
...
* __```[meta:school]```__ - группа, содержащая информацию о школе. В ней можно использовать слои со следующими ключами:
    ...
    + ```[meta:note]``` - примечание учебного заведения.
* __```[meta:class]```__ - группа, содержащая информацию о классе. В ней можно использовать слои со следующими ключами:
    + ```[meta:number]``` - номер класса (или название группы);
    + ```[meta:letter]``` - буква класса;
    + ```[meta:number letter]``` - буква и номер класса слитно (или название группы);
    ...
    + ```[meta:photo]``` - групповая фотография класса;
    + ```[meta:qr]``` - куар-код для скачивания программы оживающих фото;
...
![](../_media/design/school_class.png)
...
![](../_media/design/class_years.png)
* __```[meta:teacher] / [meta:curator]```__ - группа, содержащая информацию об учителе / классном руководителе. В ней можно использовать слои со следующими ключами:
...
![](../_media/design/teacher_curator.png)
* __```[meta:student]```__ - группа, содержащая информацию об ученике. В ней можно использовать слои со следующими ключами:
...
* __```[meta:table-frames]```__   - группа для единственного в дизайне разворота с виньетками, который содержит всех учеников.   
* __```[meta:pages-frames]```__   - группа для постраничного разворота с несколькими учениками, таких разворотов в дизайне используется несколько.
* __```[meta:classmates]```__ - группа, содержащая информацию об учениках, т.е. все группы [meta:student] должны быть расположены именно в ней. Но, ученик, для которого создается альбом должен размещаться вне этой группы.
...
* __```[meta:favorite]```__ - группа, содержащая избранные фотографии.
* __```[meta:class photo]```__ - общее фото класса.
* __```[meta:page]```__ - номер страницы альбома.
* __```[meta:mark align:center]```__ - параметр для выравнивания многострочного текста по вертикали. (Пример - для цитаты [meta:note align:center]).
* __```[meta:mark can-rotate]```__ - параметр для поворота фотографии на 90 градусов против часовой стрелки.
* Если __макет не содержит рамок__ для фотографий, то на нем будет размещен коллаж с фотографиями от 3 до 6 штук.

### Размеры макетов
* Существует два подхода к подготовке макета:
    + Макет создается в точных размерах типографии и применяется поразворотно. 
    + Размер макета может отличаться от требований типографии и применяется постранично.
* Первый вариант - это традиционный способ, дающий 100% совпадение макета и отпечатанного результата. Второй - позволяет готовить адаптивные макеты, которые подходят для любых типографий.
* Техническая особенность в подготовке универсальных макетов следующая:
    + Макет для обложки альбома и трюмо готовится без корешка и без вылетов на подворот.
    + Слой с названием ```[background]``` заполняет весь разворот с областью загиба или подрезки.
    + Слои группы ```[page][align:left]``` __для альбома__ вписываются в левую половину разворота без загиба, зоны обрезки и корешка.
    + Слои группы ```[page][align:right]``` __для альбома__ размещаются в правой половине как в пункте выше.
    + Слои группы ```[page]``` __для трюмо__ вписываются в три страницы без загиба, зоны обрезки и корешка.
* Мы рекомендуем подготавливать универсальные макеты следующих размеров:
    + Вартикальный альбом: 4961x3543px (обложка без загиба и корешка), 4961x3543px (разворот).
    + Трюмо: 7287x3543px (обложка без загиба и корешков), 7024x3437px (разворот).